...
## 1b. PROJECT IDENTITY: AD-FREE SMS BACKUP & RESTORE
- **Primary Goal:** Disrupt the legacy SMS backup utilities by releasing a beautifully designed, modern, 100% ad-free alternative.
- **Tech Stack:** 100% Kotlin with Jetpack Compose (Material You design language).
- **Architecture Mandate:** This is a strictly Android-native application. Do not attempt cross-platform bridges or iOS UI ports. 
- **Agent Rules:** 
  1. Build the UI exclusively in Jetpack Compose to ensure a native Android 14/15 feel.
  2. Implement data extraction by directly querying `Telephony.Sms.CONTENT_URI` and `CallLog.Calls`.
  3. Ensure absolutely no monetization, tracking SDKs, or AdMob are injected. Follow a frictionless tipping model.
  4. Refer to `docs/sms_app_roadmap.md` for MVP feature milestones (MMS, Contacts vCard, Cloud Sync, RCS).
